function [TckResultWLS, navSolutionsWLS] = tracking_POS_RAIM(posSV, file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
%Purpose:
...
counter_corr    = corrUpt-1 * ones(svlength,1);
    ...
    if sum(flg_pos) == svlength
       ...
       %% Positioning using WLS method with RAIM (multi-round exclusion)
        % ==== Initial WLS positioning ====
        weight_wls = CN0_CT(end,:);
        [estusr_wls, dop] = wlspos(prvec, svxyzr, estusr_wls, 1e-3, el, weight_wls);
        
        % ==== RAIM check and remove faulty satellites (iterative removal version) ====
        sigma = 3; % measurement standard deviation
        threshold = (5.33 * sigma)^2; % detection threshold
        ...
        while true
            ...
            if test_statistic < threshold
                break; % Detection passed, no faulty data, exit
            end
        
            % Detection failed, faulty data found
            [~, bad_idx] = max(abs(residual));
        
            % ---- Remove data corresponding to bad_idx ----
            prvec(bad_idx) = [];
            pseudorange(bad_idx) = [];
            svxyz(bad_idx,:) = []; % This line was missing
            svxyzr(bad_idx,:) = [];
            sv_vel(bad_idx,:) = [];
            carrFreq(bad_idx) = [];
            sv(bad_idx) = [];
            el(bad_idx) = [];
            az(bad_idx) = []; % Also remove az
            transmitTime(bad_idx) = [];
        
            % CN0_CT: remove entire column for the bad satellite
            weight_wls(bad_idx) = [];
            delayValue(bad_idx,:) = [];
        
            % Update svlength
            svlength = length(sv);
            flg_pos = zeros(1, svlength); % Reinitialize accordingly
        
                % ---- Re-estimate WLS positioning ----
            [estusr_wls, dop] = wlspos(prvec, svxyzr, estusr_wls, 1e-3, el, weight_wls);
        
            disp(['RAIM detected error, removing satellite ', num2str(bad_idx)]);
        end
        
        % ---- Check if the remaining number of satellites is sufficient ----
        if svlength < 4
            warning('Fewer than 4 satellites remaining after RAIM, skipping this epoch');
            continue;
        end
        
        % ---- Check if PDOP is too large ----
        if dop(2) > 10  % dop(2) is PDOP
            warning('PDOP too large after RAIM (%.2f), skipping this epoch', dop(2));
            continue;
        end
        
        % ==== After exclusion, recalculate Protection Level (PL) ====
        K_md = 5.33;
        
        % Extract position part of covariance matrix
        Q_pos = Q(1:3, 1:3);
        ...
        % Correct 3D protection level calculation
        ...
        % ==== Stanford Chart Calculation ====
        
        % Set alarm limit
        alarm_limit = 50; % 50 meters
        
        % Position error per epoch
        num_epochs = size(navSolutionsWLS.usrPos,1);
        position_error = zeros(num_epochs,1);
        for i = 1:num_epochs
            true_pos = cnslxyz;
            est_pos = navSolutionsWLS.usrPos(i,:);
            position_error(i) = norm(true_pos - est_pos); % Euclidean distance
        end
