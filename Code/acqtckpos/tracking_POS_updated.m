function [TckResultCT_pos, navSolutionsCT] = tracking_POS_updated(posSV, file,signal,track,cmn, Acquired,TckResult_Eph, cnslxyz,eph,sbf,solu)
%Purpose:
...
% Load sky mask data and interpolate to 1-degree resolution ====
skymask_data = readmatrix("C:\PloyU\DH\Course\Satellite\Assignment2\skymask_A1_urban.csv");
az_mask = skymask_data(:,1);  % Original azimuth angles (°)
el_mask = skymask_data(:,2);  % Corresponding elevation mask angles (°)

% Ensure azimuth covers a full circle (0° to 360°)
if az_mask(end) ~= 360
    az_mask(end+1) = 360;
    el_mask(end+1) = el_mask(1); % Ensure continuity by connecting start and end points
end

% Generate azimuth values at 1-degree intervals (0° to 359°)
az_full = 0:359;
...
%                  eph(prn).TOW(1);
        ...
        %%Adjust satellite weights based on sky mask ====
        
        % Current satellite azimuth and elevation angles
        az_now = az;
        el_now = el;
        
        % Determine visibility for each satellite using the sky mask
        visibility = zeros(1, svlength);
        for i = 1:svlength
            az_idx = mod(round(az_now(i)), 360) + 1; % Map azimuth to [1, 360]
            el_thresh = el_full(az_idx);             % Elevation threshold from sky mask
            visibility(i) = el_now(i) >= el_thresh;   % 1 if visible, 0 if blocked
        end
        
        % Base weights from latest CN0 (carrier-to-noise ratio)
        CN0_CT_base = CN0_CT(end, :);
        
        % Adjust weights: reduce weight for blocked satellites
        CN0_CT_adjusted = CN0_CT_base;
        CN0_CT_adjusted(visibility == 0) = CN0_CT_adjusted(visibility == 0) * 0.45; 
        
        %% Position cal using LS method
        if solu.mode == 1
            [estusr_wls, dop]       = wlspos(prvec,svxyzr,estusr_wls,1e-3,el,CN0_CT_adjusted);
            [VR, dtRV, ~]     = ...
